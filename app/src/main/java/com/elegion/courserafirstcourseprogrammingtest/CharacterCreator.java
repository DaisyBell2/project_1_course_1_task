...
public class CharacterCreator extends Observable implements Serializable {
    ...
    /////////////// Создала функции чтобы не дублировать один и тот же код
    private String toUpperAndLowerCase(String string) {
    ...
    private String[] getArrayString(Enum[] mEnum) {
        String[] array = new String[mEnum.length];
        String string;

        for (int i = 0; i <= mEnum.length - 1; i++) {
            string = mEnum[i].toString();
            array[i] = toUpperAndLowerCase(string);
        }
        return array;
    }

    private Enum setVariableToPosition(int position, Enum[] mEnum) {
        int lengthEnum = mEnum.length - 1;

        if (position < 0) position = 0;
        else if (position > lengthEnum) position = lengthEnum;
        return mEnum[position];
    }
    ///////////////////////////////////////////////////////////////////////
    ...
    public String[] getSpecializations() {
        ...
        return getArrayString(Specialization.values());  //new String[]{""};
    ...
    public void setSpecialization(int position) {
        ...
        mSpecialization = (Specialization) setVariableToPosition(position, Specialization.values());
    ...
    public String[] getRaces() {
        ...
        return getArrayString(Race.values());  //new String[]{""};
    }
    ...
    public void setRace(int position) {
        ...
        mRace = (Race) setVariableToPosition(position, Race.values());
    ...
    public String[] getAttributes() {
         ...
         *   One, Two, Three
         * */
        return getArrayString(Attribute.values());   //new String[]{""};
    ...
    public String[] getPerks() {
        ...
        return getArrayString(Perk.values());   //new String[]{""};
    ...
    public void updateAttributeValue(int position, int updateTo) {
        ...
        if ((currentAttrValue + updateTo >= 1) && (mAvailablePoints - updateTo) >= 0) {
            currentAttrValue += updateTo;
            currentAvailablePoints -= updateTo;

            mAttributesMap.put(attr, currentAttrValue);
            mAvailablePoints = currentAvailablePoints;

            if (Objects.equals(mAttributesMap.get(attr), currentAttrValue) && mAvailablePoints == currentAvailablePoints) {
                setChanged();
                notifyObservers();
            }
        }
